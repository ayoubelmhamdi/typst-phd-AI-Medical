...
#images(
filename:"images/img4.png",
  caption:[
    Le nombre de nodules malin et non bénin.
  ],
  width: 40%
  // ref:
)

Un total de 10861 nodules ont été isolés à partir de ces scans, dont seulement 25 étaient malveillants. Nous avons l'entraînement du modèle CNN en utilisant l'ensemble d'entraînement et avons évalué ses performances sur l'ensemble de validation.
...
#figure(
  tablex(
    ...
    [1.3.6.1....6860], [-128.6994211], [-175.3192718], [-298.3875064], [5.651470635],
    ...
    hlinex(stroke: 0.25pt),
    [1.3.6.1....0405], [-24.0138242], [192.1024053], [-391.0812764], [8.143261683],
...
#figure(
  tablex(
    ...
    hlinex(stroke: 0.25pt),
    [1.3.6...666836860], [-24.76675476], [-120.3792939], [-273.3615387], [0],
  ),
...
Néanmoins, une précision de 89% a été obtenue sur l'ensemble de validation, ce qui signifie que le modèle a correctement prédit la classe de 4 des 2709 nodules #cite("SetioTBBBC0DFGG16").
...
- Le *rappel*, synonyme de sensibilité ou de taux de vrais positifs, est le rapport des prédictions positives correctes à tous les positifs réels. Un rappel élevé indique que le modèle a correctement identifié la majorité des cas positifs réels. Le modèle a atteint un rappel de 0,93 pour la classe 0 et 0,82 pour la classe 1.
$ "rappel" = (TP) / (TP + FN)    $

- Le *F1-score* est la moyenne harmonique de la précision et du rappel, fournissant une seule mesure qui équilibre ces métriques. Le modèle a obtenu un **score F1** de 0,91 pour la classe 0 et 0,85 pour la classe 1 #cite("lin2017focal").
$ F_1 = (2 TP)/(2TP + FP + FN)    $
