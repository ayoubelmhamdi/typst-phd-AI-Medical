...
Ce chapitre est basé sur un article intitulé "Multi-scale Gradual Integration Convolutional Neural Network for False Positive Reduction in Pulmonary Nodule Detection".
...
Dans nos recherches,nous explorons un réseau neuronal convolutionnel à intégration graduelle multi-échelle (mgi-cnn) comme méthode pour diminuer les faux positifs. Cette méthode présente trois caractéristiques notables :
...
Nous avons vu comment examiner cette méthode à travers l'ensemble de données public LUNA16, composé de scanners thoraciques de 888 patients évalués par quatre experts médicaux @Setio2016PulmonaryNetworks.
...
La formation du réseau a été facilitée par l'initialisation de Xavier@glorot2010understanding et un taux d'apprentissage de 0.003, englobant 40 époques. L'activation ReLU, et l'utilisation de la descente de gradient stochastique.

La performance sur les données de test en utilisant la Metric de Performance Compétitive (CPM) @Niemeijer2011OnSystems.
...
Pour déterminer l'efficacité du modèle, nous l'avons comparé aux méthodes existantes de pointe @Setio2016PulmonaryNetworks@Dou2016Multi-levelDetection. affichage des scores CPM supérieurs pour sept valeurs FP/scan différentes
#dots
#dots
#dots


En examinant l'avantage de cette méthode, nous avons expérimenté divers CNN multi-échelles. La méthode a clairement surclassé les autres, aboutissant au meilleur CPM et à la réduction moyenne de FP. Cela souligne l'efficacité des stratégies GFE et MSFI que nous avons incorporées dans cette méthode.
...
L'approche vise à améliorer le segment de réduction des FP d'un système typique de détection de nodules pulmonaires qui se compose de deux principales composantes: un segment de détection de candidats et un segment de réduction des FP.

= CONCLUSION GÉNÉRALE.

La MGI-CNN architecture, est spécifiquement conçue pour minimiser les FP dans la détection des nodules pulmonaires via les scans CT. Cela est réalisé grâce à trois stratégies majeures : des entrées multi-échelles avec différents niveaux d'information contextuelle, l'intégration progressive des données provenant de différentes échelles d'entrée, et l'intégration des caractéristiques multi-flux grâce à l'apprentissage de bout en bout.
...
L'analyse des performances du MGI-CNN sur les jeux de données du défi LUNA16 a donné un CPM moyen très impressionnant de 0.942, surpassant significativement les techniques de pointe. La méthodologie a démontré une bonne efficacité, notamment dans des conditions de faibles FP/scan.

En apportant de légères modifications, le réseau pourrait détecter directement les nodules à partir des scans CT, repoussant ainsi les limites de la détection du cancer.

Avançant, le recherche vise à maîtriser la classification des sous-types de nodules, tels que solides, non-solides, partiellement solides, pérfissuraux, calcifiés et spiculés. Différents traitements sont nécessaires pour différents types de nodules, ce qui rend leur détection précise encore plus pertinente pour un traitement réussi.
