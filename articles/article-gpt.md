...
We combine ‘zoom-in’ and ‘zoom-out’ information streams that capture different scales of nodule morphology and context. These streams have diverse and complementary features that can enhance the nodule detection. We design a network model that integrates these features from both streams. This model uses more convolutional and fully-connected layers.



---

MSFI stands for Multi-Stream Feature Integration, which is a technique to combine features from different scales and perspectives in lung nodule detection. MSFI is part of the MGI-CNN, which is a multi-scale convolutional neural network that we developed to improve the performance of false positive reduction in lung nodule detection using the Luna16 dataset.
...
Second, we feed each stream into a convolutional neural network (CNN) that extracts features from the patches. The CNN can have multiple layers, such as convolution, pooling, and fully connected layers. The output of the CNN is a feature map that represents the characteristics of the patch.

Third, we integrate the feature maps of the two streams using MSFI. We can use different methods to combine the feature maps, such as concatenation, element-wise summation, or $1\times1$ convolution. The output of MSFI is a combined feature map that contains information from both streams.
...
Our experiments utilized the LUng Nodule Analysis 2016 (LUNA16) datasets, while the network training was initialized with Xavier's method, using a learning rate of 0.003 decreased with a weight decay in every epoch. Our network utilized a dropout technique for stability and used a Competitive Performance Metric (CPM) for performance evaluation.

## Performance Evaluation

To validate our proposed method, we compared with other existing state-of-the-art methods and found our method surpassing others in terms of Competitive Performance Metric (CPM) scores over seven different FP/scan values.

## Efficacy of Proposed Strategies

For contrasting the effects of our strategies in creating a multi-scale CNN, we experimented with other Multi-scale CNNs (MCNNs) as well. The performance of our proposed method outclassed all other methods in terms of average CPM and FP reduction, bringing out the significance of our GFE and MSFI strategies.

---
# Experiments and Results

The LUNA16 challenge datasets, comprising 888 patients with lung nodules annotated by four radiologists, were utilized. Patients with a slice thickness exceeding 2.5 mm were omitted. A nodule was deemed as Ground Truth (GT) if at least three radiologists concurred. The total number of nodules was 1186.

For the False Positive (FP) reduction task, LUNA16 supplied candidate nodule coordinates, patient IDs, and labels from commercial systems. Two versions of datasets were available: V1 contained 551065 candidates, including 1351 nodules and 549714 non-nodules, with 1120 nodules matching with GTs. V2 contained 754975 candidates, including 1557 nodules and 753418 non-nodules, with total of 1186 nodules matching with GTs.
...
The Competitive Performance Metric (CPM) score, the average sensitivity at seven FP/scan levels (0.125, 0.25, 0.5, 1, 2, 4, and 8), was used for performance evaluation.
...
### CPM Example.
Calculating the sensitivity at a false positive (FP) per scan level of 8 means that we want to find the model's sensitivity when, on average, the model produces 8 false positives per CT scan. To give an example, let's assume we have a dataset of 100 CT scans with the following results from the model:
...
Let's consider an example where we have a dataset with the following numbers of nodule and non-nodule samples:

- Nodule samples: 500
- Non-nodule samples: 2,500

First, calculate the nodule-to-non-nodule ratio by dividing the number of nodule samples by the number of non-nodule samples:

Nodule-to-non-nodule ratio = Nodule samples / Non-nodule samples = 500 / 2,500 = 1 / 5

In this example, the initial ratio is 1:5. However, we want to achieve a ratio of approximately 1:6. To do this, we can augment the nodule samples by rotating and shifting them on different planes and axes, creating new nodule samples in the process.

Let's assume that after augmentation, we now have 600 nodule samples. The new nodule-to-non-nodule ratio is:

New nodule-to-non-nodule ratio = New nodule samples / Non-nodule samples = 600 / 2,500 = 1 / 4.17 ≈ 1 / 4

Now, the ratio is closer to the desired 1:6 ratio. Further augmentation techniques can be applied to achieve a ratio even closer to 1:6 if necessary. Balancing the data in this manner ensures that the model is trained on a more representative dataset, which can help improve its performance in detecting lung nodules.

# Discussions

Lung nodule detection using deep learning techniques can help in early detection and diagnosis of lung cancer. The LUNA16 dataset is a widely used resource for developing and evaluating lung nodule detection algorithms [Source 4](https://github.com/kshannon/lung-nodule-localization). In this context, we propose an MGI-CNN architecture that has two main advantages:
...
For the MC group, the false positives had low contrast, possibly due to normalization during preprocessing. This suggests that network performance could be improved by using more patches with different scales and normalization methods.

The study focused on improving the FP reduction part of a pulmonary nodule detection system, which typically consists of a candidate detection part and an FP reduction part. The proposed network can work with any candidate detection method, as it is independent of it. Combining the network with more advanced candidate detection methods is expected to yield better results.
